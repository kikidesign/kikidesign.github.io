...
一旦您的分支已经创建，它的时间开始改变。无论何时添加、编辑或删除一个文件，您都将提交一个提交，并将它们添加到您的分支中。这个过程中添加的提交跟踪您的进展，因为您在一个功能分支工作。
也创造了一个透明的历史，您的工作，其他人可以遵循，以了解您做什么，为什么。每一个提交都有一个相关的提交信息，这是一个描述为什么一个特定的变化的说明。此外，每一个提交被认为是一个单独的变更单元。这让您回滚更改，如果一个错误被发现，或者如果您决定在一个不同的方向。
...
如果您正在使用叉子和拉模型，拉请求提供了一种方式来通知你希望他们考虑改变项目的维护者。
如果您正在使用共享存储库模型，拉请求会帮助启动代码审查，并在合并到主分支之前讨论更改。
...
![Deployment-Target](https://raw.githubusercontent.com/kikidesign/kikidesign.github.io/master/_image/GitHub-Flow.png)
